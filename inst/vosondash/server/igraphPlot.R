...
igraphData <- reactive({
  ...
  if (is.null(g)) {
    return(emptyPlotMessage("No graph data."))
  }
  if (vcount(g) <= 0) {
    return(emptyPlotMessage("No vertices to plot."))
  }
  ...
  if ("color" %in% vertex_attr_names(g)) {
    v_color_in_data <- TRUE
  }
  
  if (input$use_vertex_colors_check == FALSE) {
    # added checkbox
  ...
  if (length(categorical_attributes) > 0) {
    # only if have categorical attributes
    
    if (nchar(selected_categorical_attribute) &&
        selected_categorical_attribute != "All") {
      categories <-
        categorical_attributes[[selected_categorical_attribute]]
  ...
  plot_parameters[['vertex.color']] <-
    ifelse(V(g)$id %in% selected_row_names,
           gbl_plot_sel_vertex_color,
           V(g)$color)
  plot_parameters[['vertex.frame.color']] = ifelse(V(g)$id %in% selected_row_names, "#000000", "gray")
  plot_parameters[['vertex.label.font']] <-
    ifelse(V(g)$id %in% selected_row_names, 2, 1)
  ...
  if (input$edge_labels_check == TRUE &
      !is.null(input$edge_label_select)) {
    plot_parameters['edge.label.cex'] <- input$edge_label_size
    plot_parameters[['edge.label']] <-
      edge_attr(g, input$edge_label_select)
  }
  ...
  if (node_index_check == FALSE) {
    ...
    plot_parameters[['vertex.label.color']] = ifelse(
      V(g)$id %in% selected_row_names,
      gbl_sel_label_col,
      ifelse(
        is.null(V(g)$label.color),
