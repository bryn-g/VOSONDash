  ...
  if (isLocal) {
    isolate({
      if (file.exists(u_api_keys_path)) {
        ...
        if (load_and_use_keys) {
          ...
          updateTextInput(
            ...
            label = NULL,
            value = api_keys$twitter_api_key
          )
          updateTextInput(
            session,
            "twitter_api_secret_input",
            label = NULL,
            value = api_keys$twitter_api_secret
          )
          updateTextInput(
            session,
            "twitter_access_token_input",
            label = NULL,
            value = api_keys$twitter_access_token
          )
          updateTextInput(
            session,
          ...
          updateTextInput(
            ...
            label = NULL,
            value = api_keys$youtube_api_key
          )
          
          creds_rv$msg_log <-
            logMessage(creds_rv$msg_log, "loaded and populated api keys")
        }
      }
      ...
      if (file.exists(u_api_tokens_path)) {
        ...
        creds_rv$msg_log <-
          logMessage(creds_rv$msg_log,
                     paste("loaded tokens from file", u_api_tokens_path))
        ids <- getTokenIds()
        updateSelectInput(
          session,
        ...
        creds_rv$msg_log <-
          logMessage(creds_rv$msg_log,
                     paste("no tokens file found", u_api_tokens_path))
      }
    })
...
observeEvent(input$create_app_token, {
  keys <- list(
    apiKey = input$keys_twitter_api_key_input,
  ...
  tryCatch({
    creds_rv$created_token <-
      VOSONDash::createTwitterDevToken(input$keys_twitter_app_name_input, keys)
  }, error = function(err) {
    creds_rv$msg_log <-
...
observeEvent(input$create_web_auth_token, {
  ...
  tryCatch({
    creds_rv$created_token <-
      VOSONDash::createTwitterWebToken(input$keys_twitter_app_name_input, keys)
  }, error = function(err) {
    creds_rv$msg_log <-
      logMessage(creds_rv$msg_log, paste("token creation error:", err))
    creds_rv$created_token <- NULL
...
observeEvent(input$tokens_save_button, {
  if (isLocal) {
    ...
    creds_rv$msg_log <-
      logMessage(creds_rv$msg_log,
                 paste0("saved tokens to file (", length(creds_rv$tokens), ")"))
  }
})
...
observeEvent(creds_rv$created_token, {
  if (is.null(creds_rv$created_token)) {
    creds_rv$msg_log <-
      logMessage(isolate(creds_rv$msg_log), "created invalid token")
...
observeEvent(input$save_token, {
  ...
  if (!is.null(token)) {
    ...
    creds_rv$msg_log <-
      logMessage(creds_rv$msg_log, "saved token to list")
    
    creds_rv$selected_token_id <- token_id
  } else {
    creds_rv$msg_log <-
      logMessage(creds_rv$msg_log, "unable to save token to list")
  }
})
...
observeEvent(input$use_selected_token, {
  if (input$twitter_token_select == "None") {
    ...
    creds_rv$use_token <-
      creds_rv$tokens[[input$twitter_token_select]]
    creds_rv$msg_log <-
      logMessage(creds_rv$msg_log,
                 paste("using token", input$twitter_token_select))
...
observeEvent(input$delete_selected_token, {
  if (input$twitter_token_select %in% names(creds_rv$tokens)) {
    creds_rv$tokens <-
      creds_rv$tokens[!grepl(input$twitter_token_select,
                             names(creds_rv$tokens),
                             fixed = TRUE)]
    
    creds_rv$msg_log <-
      logMessage(creds_rv$msg_log,
                 paste("deleted", input$twitter_token_select))
...
writeKeysFile <- function() {
  if (isLocal) {
    ...
    creds_rv$msg_log <<-
      logMessage(creds_rv$msg_log, paste("wrote keys to", u_api_keys_path))
...
readKeysFile <- function() {
  ...
  if (file.exists(u_api_keys_path)) {
    creds_rv$msg_log <<-
      logMessage(creds_rv$msg_log,
                 paste("file", u_api_keys_path, "exists"))
    ...
    creds_rv$msg_log <<-
      logMessage(creds_rv$msg_log,
                 paste("file", u_api_keys_path, "not found"))
  ...
  updateCheckboxInput(
    ...
    label = NULL,
    value = api_keys$load_and_use_keys
  )
...
populateYoutubeKeys <- function() {
  updateTextInput(
    ...
    "youtube_api_key_input",
    label = NULL,
    value = input$keys_youtube_api_key_input
  )
  
  creds_rv$msg_log <<-
    logMessage(creds_rv$msg_log, "populated youtube api keys")
}
