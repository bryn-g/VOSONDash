...
if (isLocal) {
  message(
    ...
    paste0(
      "VOSONDash",
      " v",
      utils::packageVersion("VOSONDash"),
      " (",
    ...
    paste0(trimws(
      paste(Sys.getenv("os"), R.Version()$platform)
    ), "\n"),
...
if (pkgMsgs == FALSE) {
  ...
  loadedPackages <-
    sapply(requiredPackages, function(x) {
...
if (isLocal) {
  if (any(loadedPackages == FALSE)) {
    ...
    err_msg <- paste0(
      ...
      paste0(sapply(missingPackages, function(x)
        paste0("- ", x)),
        collapse = "\n"),
    ...
    packageStr <-
      sapply(missingPackages, function(x)
        paste0("\"", x, "\""))
    err_msg <- paste0(err_msg,
                      paste0(
                        "install.packages(c(",
                        paste0(packageStr, collapse = ","),
                        "))\n"
                      ),
                      "\n")
