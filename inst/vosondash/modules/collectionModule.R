...
collectDataButtonsUI <- function(id) {
  ...
  tagList(shinyjs::disabled(
    downloadButton(ns("dl_data"), label = "Data", title = "Download Raw Data File")
  ))
}
...
collectNetworkButtonsUI <- function(id) {
  ...
  tagList(shinyjs::disabled(
    downloadButton(ns("dl_network"), label = "Network", title = "Download Network Data File")
  ))
}
...
collectGraphButtonsUI <- function(id) {
  ...
  tagList(shinyjs::disabled(
    downloadButton(ns("dl_graph"), label = "Graphml", title = "Download Network Graphml File")
  ))
}
...
collectDataButtons <-
  function(input, output, session, data, file_prefix = "") {
    output$dl_data <- downloadHandler(
      filename = function() {
        systemTimeFilename(paste0(ifelse(
