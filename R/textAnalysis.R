...
#' 
#' @param tokens Dataframe of word tokens from text data.
...
#' 
#' @export
wordFreqChart <- function(tokens, min_freq = 1, top_count = 20, pcolors = NULL) {
  ...
  tokens <- tokens %>%
    ...
    dplyr::filter(.data$n >= min_freq) %>%
    dplyr::slice_max(.data$n, n = top_count)
  
  tokens$word <- factor(tokens$word, levels = tokens$word)
  
  if (nrow(tokens) > top_count+10) {
    tokens <- tokens %>% dplyr::slice_head(n = top_count+10)
  }
  ...
  # ft <- freq_terms[order_terms[1:top_count]]
  # df <- data.frame(word = names(ft), freq = unname(ft))
  p <- ggplot2::ggplot(data = tokens, aes(x = forcats::fct_rev(word), y = n))
  if (is.null(pcolors)) {
    p <- p + geom_bar(stat = "identity")
  } else {
    p <- p + geom_bar(stat = "identity", fill = pcolors)
  }
...
#' 
#' @export
wordCloudPlot <- function(tokens, seed = NULL, min_freq = 1, max_words = 50, pcolors = NULL) {
  ...
  tokens <- tokens %>%
    dplyr::count(.data$word, sort = TRUE) # %>%
    # dplyr::filter(.data$n >= min_freq) # %>%
    # dplyr::slice_max(.data$n, n = max_words)

  # p <- wordcloud2::wordcloud2(tokens, size = max_words) # , color = pcolors
  # return(p)
  ...
  par(mar = rep(0, 4))
  wordcloud::wordcloud(tokens$word, tokens$n, 
                       min.freq = min_freq, 
                       max.words = max_words, 
                       random.order = FALSE, 
                       colors = pcolors)  
